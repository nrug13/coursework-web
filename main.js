...
function selected(defID = 0) {
  selectTag.forEach((tag, id) => {
    ...
    for (const country_code in countries) {
      ...
      if (
        (id == defID && country_code == "en-GB") ||
        (id != defID && country_code == "az-AZ")
      ) {
...
async function Translate(text, translateFrom, translateTo) {
